/*  Communicate with Particle Variables and Functions in Vanilla JS 
    ...
    This example uses api.php that will serve as a proxy. 
    This hides the access_token and the deviceID in the PHP code, so it's not 
    visible for the end-user. The problem is that a request will take more time. 
    
    Copyleft 28-11-2017 - http://www.kasperKamperman.com
*/
...
function callParticleFunction(functionName, command, callbackFunction) {
    ...
    var url    = "api.php?func="+functionName; //+"/";
    //var params = "access_token="+accessToken+"&arg="+command;
    var params = "arg="+command;
...
function callParticleVariable(variableName, callbackFunction) {
    ...
    var url    = "api.php";//?var="+variableName; //+"/";
    ...
    // if we have a name, add it. Otherwise we get the device information. 
    if(Boolean(variableName)) {
        params = "var="+variableName;    
    }
